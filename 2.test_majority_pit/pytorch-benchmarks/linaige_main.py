...
def main(args):
    ...
    for dataset in ds_linaige_cv:
        ...
        if majority_win != None and win_size == 1:
            test_metrics = train_module.evaluate(exported_model, criterion, ds_majority, device, classification, class_number, search, majority_win)
        else:
            test_metrics = train_module.evaluate(exported_model, criterion, ds_test, device, classification, class_number, search, majority_win)
    ...
    file_path = f'model_pit_majority_{model_name}_{channel1}_{channel2}_reset_code_{reset_code_flag}.xlsx'
