...
def main(args):
    ...
    # print('***********************************************This is specific to TNC model with new PLINIO file with window = 3 ***********************************************')
    ...
    input_shape = (1, 8, 8)
    example = [torch.stack([torch.rand(input_shape)] * 2, 0) for _ in range(3)]
        
    pit_model = PIT(model_base, input_example=example)
    pit_model = pit_model.to(device)
    pit_model.train_features = True
    pit_model.train_rf = False
    pit_model.train_dilation = False
    print(summary(pit_model, [(1, 8, 8), (1, 8, 8), (1, 8, 8)]))
    ...
    for dataset in ds_linaige_cv:
        ...
        print(summary(exported_model, [(1, 8, 8), (1, 8, 8), (1, 8, 8)]))     
    ...
    file_path = f'model_pit_tcn_{model_name}_{channel1}_{channel2}_reset_code_{reset_code_flag}.xlsx'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default=None, help='Model List:[c_p_c_tcn_fc] specific to TCN model')
