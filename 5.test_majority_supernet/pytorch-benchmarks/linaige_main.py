...
def main(args):
    ...
    for epoch in range(N_EPOCHS):
        metrics = train_module.train_one_epoch(epoch, model_base, criterion, optimizer, ds_train, device, classification, class_number, search, cd_size=None, cd_ops=None)
    ...
    search = True
    for epoch in range(N_EPOCHS):
        metrics = train_module.train_one_epoch(epoch, nas_model_supernet, criterion, optimizer, ds_train, device, classification, class_number, search, cd_size=cd_size, cd_ops=cd_ops)
        # Checking if there is not any imporvement in metrics to reduce the learning rate, the patient is 5 epochs
        reduce_lr.step(metrics['loss']) # changed to scheduler.step()???
        # Checking if there is not any imporvement in metrics, the patient is 10 epochs
        if earlystop(metrics['loss']): # eliminated ???
    ...
    # search = True
    # for epoch in range(N_EPOCHS):
    #     metrics = metrics = train_module.train_one_epoch(epoch, pit_model, criterion, optimizer, ds_train, device, classification, class_number, search, cd_size=cd_size, cd_ops=cd_ops)
    ...
    for dataset in ds_linaige_cv:
        ...
        search = False
        for epoch in range(N_EPOCHS):
            metrics = train_module.train_one_epoch(epoch, exported_model, criterion, optimizer, ds_train, device, classification, class_number, search, cd_size=None, cd_ops=None)
            # Checking if there is not any imporvement in metrics to reduce the learning rate, the patient is 5 epochs
            reduce_lr.step(metrics['loss']) # changed to scheduler.step()???
            # Checking if there is not any imporvement in metrics, the patient is 10 epochs
            if earlystop(metrics['loss']): # eliminated???
        ...
        if majority_win != None and win_size == 1:
            test_metrics = train_module.evaluate(exported_model, criterion, ds_majority, device, classification, class_number, majority_win)
        else:
            test_metrics = train_module.evaluate(exported_model, criterion, ds_test, device, classification, class_number,  majority_win)
    ...
    data = {
        ...
        'SUPERNET Model Total Param': [sum(p.numel() for p in exported_model.parameters())],
    ...
    file_path = f'model_{model_name}_{channel1}_{channel2}_w_{win_size}_reset_code_{reset_code_flag}.xlsx'
